...
/* 오브젝트 매니져가 가지고 있는 객체들은 사용자가 필요하따라서 그룹으로 묶어서 관리하고 있기때문에 더해서 렌더순서까지 관리해주기에는 버겁다. */
...
/* 스카이박스 : 스카이박스는 모두에게 덮여서 그려져야한다. 스카이박스의 픽셀 당 깊이를 다른 객체들의 깊이와 비교 못하게 해야할 필요가 있었다. */
/* 2가지 : 1번째 방법 스카이박스를 가장 마지막에그리면서 깊이 비교를 끄는 방법.깊이비교를 끄고 그리면 무조건 덮고 그린다. => 하늘이 다른애들을 다 덮는다. (X) */
/* 2가지 : 2번째 방법 스카이박스를 가장 먼저 그리면서 깊이를 기록해놓지 않는 방법 (O) */
/* 블렌드 : 모든 불투명한 애들 다음에 그린다. 블렌드(이미 그려져있던 픽셀의 색과 그릴려고하는 픽셀의 색을 섞는다.)그룹의 객체들을 그리기전에 모든 불투명한 객체들을 그려놨어야 잘 섞인다.  */
/* UI : 모든 픽셀을 다 덮고 그려낸다. 가급적 가장 마지막에 그려내되 깊이비교를 끄고 그린다. 삼차원공간에 있는 유아이다? 깊이테스트가 필요해질수 있어. 논블렌드 그룹으로 가서 그려지면된다. */
...
{
public:
	enum RENDERGROUP { RG_PRIORITY, RG_SHADOW, RG_NONBLEND, RG_NONLIGHT, RG_BLEND, RG_UI, RG_END };
private:
