...
#include "Weapon.h"
#include "MonsterHP.h"
CMecanoBot::CMecanoBot(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CActor{pDevice, pContext}
{
}

CMecanoBot::CMecanoBot(const CMecanoBot& Prototype) : CActor{Prototype}
{
}
...
void CMecanoBot::NON_intersect(_float fTimedelta)
{
    ...
    if (40.f < fLength)
    {
        m_pGameInstance->Add_Job(
            [this]()
            {
                m_pNavigationCom->Set_Taget(
                    m_pGameInstance->Get_Player()->Get_Transform()->Get_TRANSFORM(CTransform::T_POSITION));
            });

        m_pTransformCom->Move_TagetAstar(m_pNavigationCom, fTimedelta);
    }
    if (40.f > fLength && 15.f < fLength)
    {
        _float3 fDir{};
...
HRESULT CMecanoBot::Bind_ShaderResources()
{
    return S_OK;
}
