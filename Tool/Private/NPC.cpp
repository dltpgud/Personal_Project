...
CNPC::CNPC(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CGameObject{pDevice, pContext}
{
  
}

CNPC::CNPC(const CNPC& Prototype) : CGameObject{Prototype}
{
}
...
_int CNPC::Priority_Update(_float fTimeDelta)
{
    if (m_bDead)
        return OBJ_DEAD;
...
_tchar* CNPC::Get_ProtoName()
{
    return m_Proto;
}

_float CNPC::check_BoxDist(_vector RayPos, _vector RayDir)
{
